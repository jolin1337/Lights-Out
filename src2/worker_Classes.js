...
var color=function(r,g,b){
	...
	this.mixColor = function(c){
		return new color(
			Math.min(this.r +  c.r,255),
			Math.min(this.g +  c.g,255),
			Math.min(this.b +  c.b,255)
		);
	};
...
var Object3D = function(){
	...
	scope.applyPosition=function(){
		for(var f = 0; f<scope.Faces.length; f++)
			for(var v = 0; v<scope.Faces[f].vertices.length; v++)
				scope.Faces[f].vertices[v].position.add(scope.position);
		scope.position = new Vector(0,0,0);
	};
};
